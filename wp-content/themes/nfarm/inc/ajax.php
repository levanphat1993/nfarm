...
add_action( 'wp_ajax_nopriv_nfarm_save_user_contact_form', 'nfarm_save_contact' );
add_action( 'wp_ajax_nfarm_save_user_contact_form', 'nfarm_save_contact' );
...
function nfarm_save_contact()
{
    ...
    if ($postID !== 0) {
        ...
        $subject = 'Nfarm Contact Form - '.$title;

        $headers[] = 'From: '.get_bloginfo('name').' <'.$to.'>'; // 'From: Dev <me@devmaster.com>'
